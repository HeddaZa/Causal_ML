...
class extendedERUPT:
    def __init__(
        ...
        self.df_metric["test_set_treatment"] = self.df_metric[
            "test_set_treatment"
        ].astype(int)
        self.df_metric["target test"] = target_test

    def _add_random_treatment_col(self, random_state: int):
        """
        ...
        random_state : int
        """
        self.df_metric["random"] = (
            self.df_metric["best treatment"]
            .sample(n=self.df_metric.shape[0], replace=False, random_state=random_state)
            .values
        )

    def _get_common_treatment_index(self, col_to_compare: str) -> pd.Series:
        """
        ...
        """
        return self.df_metric.loc[
            self.df_metric[col_to_compare] == self.df_metric["test_set_treatment"]
        ].index

    @staticmethod
    def p_value(distribution: np.ndarray, value: float) -> float:
        """
    ...
    def _evaluate_random_distr(self):
        ...
        for xx in range(500):
            ...
            erupt_value_bench = (
                self.df_metric["target test"].loc[index_erupt_bench].sum()
                / self.df_metric["target test"].loc[index_erupt_bench].shape[0]
            )

            erupt_bench_values.append(erupt_value_bench)
            erupt_bench_sum_sucess.append(
                self.df_metric["target test"].loc[index_erupt_bench].sum()
            )
            erupt_bench_sum_match.append(
                self.df_metric["target test"].loc[index_erupt_bench].shape[0]
            )

        return (
            np.mean(erupt_bench_values),
            np.mean(erupt_bench_sum_sucess),
            np.mean(erupt_bench_sum_match),
            erupt_bench_values,
        )

    def get_ERUPT_with_benchmark(self):
        """
        ...
        erupt_value_test = (
            self.df_metric["target test"].loc[index_erupt_test].sum()
            / self.df_metric["target test"].loc[index_erupt_test].shape[0]
        )

        (
            erupt_bench_values_mean,
            erupt_bench_sum_sucess_mean,
            erupt_bench_sum_match_mean,
            erupt_bench_values,
        ) = self._evaluate_random_distr()

        erupt_value_bench_2 = self.df_metric["target test"].mean()

        print(
            f'ERUPT model: {erupt_value_test:.4f}, sum of y: {self.df_metric["target test"].loc[index_erupt_test].sum()}, number of rows: {self.df_metric["target test"].loc[index_erupt_test].shape[0]}'
        )

        print(
            f"ERUPT benchmark: {erupt_bench_values_mean:.4f}, sum of y: {round(erupt_bench_sum_sucess_mean)}, number of rows: {round(erupt_bench_sum_match_mean)}"
        )
        print(
            f"Difference of ERUPT(model) and ERUPT(benchmark): {erupt_value_test - erupt_bench_values_mean:.4f}"
        )
