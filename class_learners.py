...
class Learner:
    ...
    def __init__(self, features, treatment, target, test_split=0.5, random_state=42):
        ...
        features_and_treatment = pd.concat(
            [features, dummy_treatment, treatment], axis=1
        )
    ...
    def get_best_treatment(self) -> pd.Series:
        """
...
class TLearner(Learner):
    ...
    def _prepare_data_T(self):
        ...
        self.X_per_segment = {}
        for j, name in enumerate(self.dummy_name):
            self.X_per_segment[name] = self.segment_split_T(
                self.X_train, self.y_train, j
            )
    ...
    def _run_all_predictions(self, classifier, treatment_name, **kwrds):
        ...
        **kwrds: 
        ...
        for key in self.dummy_name:
            ...
            clf = self._run_predictions(
                classifier,
                self.X_per_segment[key][0],
                self.X_per_segment[key][1],
                **kwrds
            )

            self.proba_per_segment[key] = clf.predict_proba(X_test_wo_tr)[:, 1]
...
class CorrSTLearner(Learner):
    ...
    def _filter_and_split(self):
        ...
        for name in self.dummy_name:
            ...
            self.y_per_segment[name] = self.y_train.loc[
                self.X_per_segment[name].index
            ].copy()
    ...
    def get_proba(self, **kwrds):
        ...
        **kwrds:
        ...
        for key in keys_segment:
            clf = self._run_predictions(
                self.X_per_segment[key], self.y_per_segment[key], **kwrds
            )
